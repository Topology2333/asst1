...
extern "C" {
void ISPCLaunch(void **handlePtr, void *f, void *data, int countx, int county,
                int countz);
...
inline TaskGroupBase::TaskGroupBase() {
  ...
  for (int i = 0; i < MAX_TASK_QUEUE_CHUNKS; ++i) taskInfo[i] = nullptr;
}
...
static inline void FreeTaskGroup(TaskGroup *tg) {
  ...
  for (int i = 0; i < MAX_FREE_TASK_GROUPS; ++i) {
    if (freeTaskGroups[i] == nullptr) {
      void *ptr = lAtomicCompareAndSwapPointer((void **)&freeTaskGroups[i], tg,
                                               nullptr);
...
void ISPCLaunch(void **taskGroupPtr, void *func, void *data, int count0,
...
class TaskSys {
 ...
 public:
  ...
  static inline void init() {
    if (global) return;
    pthread_mutex_lock(&mutex);
    if (global == nullptr) global = new TaskSys;
    pthread_mutex_unlock(&mutex);
  }
